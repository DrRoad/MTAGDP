...
##
    ...
    rgdp_pop <- ImportTS2(TRED, "Gross domestic product, by region and industry (Annual-Mar)", 
                     ...
                     filter(RGDP_industry %in% c("Gross Domestic Product"),
                           !RegionGDP %in% c("New Zealand", "Total South Island", "Total North Island")) %>%
                     select(-RGDP_industry)

 #      load("data_intermediate/tmp.gts.rda")
 #      load("data_intermediate/tmp.gts.fc.rda")
...
##
    # extract the bottom level of the time series objects we made earlier
       tg.b    <- aggts(tmp.gts,    levels = c(1)) # actuals
       tg.fc.b <- aggts(tmp.gts.fc, levels = c(1)) # forecasts

     # combine the objects into a single time series
       gts.botto <- ts(rbind(tg.b, tg.fc.b), start=c(2000, 1), frequency=12)

     # aggregate the 'monthly' time series for total gdp to yearly
       botto.ta <- ta(gts.botto, conversion="sum", to = "annual")
                
       botto.ta.df <- data.frame(matrix(botto.ta, ncol = ncol(botto.ta))) 

       names(botto.ta.df) <- colnames(botto.ta)
     
       botto.ta.df <- botto.ta.df %>%
                      mutate(Year = (seq(1:nrow(botto.ta.df)) + 1999)) 

       botto.ta.gt <- botto.ta.df %>%
...
##
    # combine mtagdp summary with the forecast data 
      # data_to_grunt <- TAGDP_defl %>%
                       # group_by(Year, TA_Region_modified, TA, Region, RegionGDP) %>%
                       # summarise(GDP = sum(GDP)) %>%
                       # right_join(botto.ta.gt, by = c("Year", "TA_Region_modified")) %>%
                       # tidyr::fill(TA_Region_modified, TA, Region, RegionGDP)
      ...
      data_to_grunt <- botto.ta.gt %>%                       
       ...
       # the original TAGDP_defl object.
        ...
        if(outby > 0){
            ...
            stop("GDP from mtagdp should match the actuals in the forecast object; something has gone wrong in the merging")
         }
...
##
    ...
    # Unlike grunt.r, no need for a loop as only one set of population totals to rake to.
      mtagdp_svy <- rake(mtagdp_svy,
                     sample.margins     = list(~Year + RegionGDP),
    ...
    outby <- with(filter(forecast_grunt, Year < 2014), sum(abs(GDP_rake - GDP), na.rm = TRUE))
    if(outby > 0.01){
    ...
    # create a final object for publication
      mtagdp_totals <- forecast_grunt %>%
        ...
        mutate(notes = ifelse(Year < 2014, "", "provisional")) %>%
        ungroup()
