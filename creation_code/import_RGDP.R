...
# into primary and other, but Northland and Southland don't - so we need to have 'Gisborne manufacturing' 
...
# All this then gets sourced from the data_raw/concordances/industries.csv file.
    rgdp_pop_pub_det <- ImportTS2(TRED, "Gross domestic product, by region and industry (Annual-Mar)", 
                             stringsAsFactors = FALSE) %>%
         rename(RegionGDP = CV2,
                  i = CV3,  # short name for this column as we need to use it lots
                  Freq = Value) %>%
           mutate(Year = as.numeric(substring(TimePeriod , 1, 4))) %>%
           select(Year, RegionGDP, i, Freq) %>%
           ...
           filter(!(i == "Manufacturing" & ! (RegionGDP %in% c("Northland", "Southland")))) %>%
  
           # No 'FFMEGWWS' unless West Coast or Marlborough:
           filter(!(i == "Forestry, Fishing, Mining, Electricity, Gas, Water and Waste Services" &
                      !(RegionGDP %in% c("West Coast", "Marlborough")))) %>%
           mutate(RegionIndustryRGDP15 = ifelse(i %in% 
                                                  c("Agriculture", 
                                                    "Construction",
                                                    "Wholesale Trade", 
                                                    "Retail Trade",
                                                ...
                                                paste(RegionGDP, i))) %>% 
           group_by(Year, RegionGDP) %>%
           filter(!(i %in% c("Gross Domestic Product", "Total All Industries"))) %>%
           filter(!is.na(Freq)) %>%
           select(-i)
