...
For estimating MTAGDP, several data sources were used to successively rake to align totals from finer to coarser-level information of geographic regions and industry classifications.  In effect, the employee and earnings numbers are weighted up to match the published GDP totals, providing a robust method for estimating GDP.  This is a viable approach because ratios of earnings to GDP in different combinations of regions and industries can be determined from the published data in national GDP, regional GDP, and the Linked Employer Employee Database.
 ...
 \begin{tabular}{ p{6cm} | p{8 cm} }
      ...
      \hline
      Business Demography Statistics & two-way table of employee numbers, Territorial Authority by fine level (i.e. 6 digit ANZSIC). \url{http://nzdotstat.stats.govt.nz} \\
      \hline
      ...
      \hline
      National Gross Domestic Product Production Measure, nominal & Provides more detailed industry breakdown than NGDP but no regional information. \url{http://www.stats.govt.nz/infoshare} \\
      \hline
      Regional Gross Domestic Product & Provides regional GDP by industry up to 2012 (at the time of writing).  National totals match (within rounding error) those in NGDP. \url{http://www.stats.govt.nz/infoshare}\\
      \hline
 \end{tabular}	
...
Final output was compiled as a separate series into a single \verb|.cvs| file to accompany the MTAGDP file with the industry-level information.  Future incorporation of these data into the shinyapp is planned for 2016.
...
\begin{tabular}{ p{6cm} | p{8cm} }
	\textbf{File in the `testing\_code' Folder}	& \textbf{Description} \\
     ...
     \hline
 	 troubleshoot\_incompatible\_population\_data.R & Script makes comparisons between marginal totals from the different data sources for Regional and National GDP and LEED tables.  Individual tests are used to validate sources used in the stages of calculating MTAGDP. \\
     \hline
